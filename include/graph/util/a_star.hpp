...
namespace util::AStar {
    ...
    static std::vector<common::Node*> getPath(common::Graph* graph, int startId, int endId, HeuristicFunc heuristic) {
        if (!graph) return {};
    ...
    static std::pair<bool, std::pair<double, double>> getCoords2D(common::Node* n) {
    ...
    static std::pair<bool, std::tuple<double, double, double>> getCoords3D(common::Node* n) {
    ...
    static double zeroHeuristic(common::Node*, common::Node*) { return 0.0; }

    static double euclideanHeuristic2D(common::Node* a, common::Node* b) {
    ...
    static double euclideanHeuristic3D(common::Node* a, common::Node* b) {
    ...
    static double manhattanHeuristic2D(common::Node* a, common::Node* b) {
        auto [hasA, posA] = getCoords2D(a);
        auto [hasB, posB] = getCoords2D(b);
        if (!hasA || !hasB) return 0.0;
    ...
    static double manhattanHeuristic3D(common::Node* a, common::Node* b) {
        auto [hasA, posA] = getCoords3D(a);
        auto [hasB, posB] = getCoords3D(b);
        if (!hasA || !hasB) return 0.0;
        auto [x1, y1, z1] = posA;
